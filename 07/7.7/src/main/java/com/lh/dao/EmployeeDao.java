...
public List<Employee> selectAllEmp(){
	...
	try{
		...
		CallableStatement stmt = conn.prepareCall("{call selectAllEmp()}");//调用存数过程
		ResultSet rs = stmt.executeQuery();//执行存储过程并返回结果集
		while(rs.next()){//循环判断结果集中是否有下一行数据
			Employee emp = new Employee();//创建Employee对象
			emp.setEmpId(rs.getInt("id"));
			emp.setEmpName(rs.getString("name"));
			emp.setEmpAge(rs.getInt("age"));
			emp.setEmpSex(rs.getString("sex"));
			emp.setEmpDuty(rs.getString("duty"));
			list.add(emp);//所有Employee对象保存到List集合中
		}
	}
	catch(Exception ex){
		ex.printStackTrace();
