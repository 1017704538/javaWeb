...
<script language="javascript">
	/**************实时获取剩余时间***********************************************/
	//此处需要加&nocache="+new Date().getTime()，否则将剩余时间不更新的情况
	function getRemainTime() {
		var loader = new net.AjaxRequest(
				"AuctionServlet?action=getRemainTime&id=${requestScope.id}&nocache="
	...
	function deal() {
		var remain = this.req.responseText; //获取剩余时间
		//remain.replace(/\s/g, ""); //用于去除字符串中的Unicode空白符
	...
	function getBidInfo() {
		var loader = new net.AjaxRequest(
				"AuctionServlet?action=getBidInfo&id=${requestScope.id}&nocache="
						+ new Date().getTime(), deal_getBid, onerror, "GET");
	}
	function deal_getBid() {
		var h = this.req.responseText; //获取出价信息
	...
	function getBidList() {
		var loader = new net.AjaxRequest(
				"AuctionServlet?action=getBidList&id=${requestScope.id}&nocache="
						+ new Date().getTime(), deal_getBidList, onerror, "GET");
	}
	function deal_getBidList() {
		var doc = this.req.responseXML; //获取出价记录
