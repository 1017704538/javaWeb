...
public class ChartAction3D extends ActionSupport {
    private JFreeChart chart = null; // struts2的固定写法
    
    public JFreeChart getChart() {
        chart = ChartFactory.createPieChart3D("IT行业市场占有率调查图", // 图标标题
                getDataset(), // 数据集
                true,// 是否显示图例
                false, // 是否显示工具提示
                false); // 是否生成url
        ...
        plot3D.setStartAngle(150D);//设置开始角度
        plot3D.setDirection(Rotation.CLOCKWISE);//顺时针方向
        plot3D.setLabelGenerator(new StandardPieSectionLabelGenerator("{0}{2}",
                NumberFormat.getNumberInstance(), NumberFormat
                        .getPercentInstance())); // {0}表示行业名，{1}表示具体值，{2}表示所占的百分比
        plot3D.setLabelFont(new Font("宋体", Font.ITALIC, 12)); // 设置饼图样式
        plot3D.setForegroundAlpha(0.5f); // 设置前景
        plot3D.setNoDataMessage("无数据显示");
        return chart;
    }
