...
public boolean updateEmployee(Employee emp){
	...
	try{
		con = DBCon.getConn();//建立数据库连接
		String sql = "update tb_employee set age=? where id=? ";//update语句
		PreparedStatement stmt = con.prepareStatement(sql);//预编译SQL语句
		stmt.setInt(1, emp.getEmpAge());//设置SQL语句第1个“?”的参数值
		stmt.setInt(2, emp.getEmpId());//设置SQL语句第2个“?”的参数值
		int i = stmt.executeUpdate();//执行修改操作,返回影响的行数	
		if(i==1) result = true;//修改成功，返回true
	}catch(Exception ex){
		ex.printStackTrace();
	}finally{
		try {
...
public boolean deleteEmployeeById(int id){
	...
	try{
		con = DBCon.getConn();//建立数据库连接
		String sql = "delete from tb_employee  where id=? ";//delete语句
		PreparedStatement stmt = con.prepareStatement(sql);//预编译SQL语句
		stmt.setInt(1,id);//设置SQL语句第1个“?”的参数值
		int i = stmt.executeUpdate();//执行删除操作,返回删除所影响的行数	
		if(i==1) result = true;//删除成功，返回true
	}catch(Exception ex){
		ex.printStackTrace();
	}finally{
		try {
