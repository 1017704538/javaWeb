...
<title>${sessionScope.account}的博客后台管理-文章修改</title>
<style type="text/css">
<!--
.style1 {color: #FFFFFF}
-->
  ...
  <tr>
    ...
    <td width="644" align="center" valign="top" bgcolor="#FFFFFF"><br>    
      ...
      <table width="550" height="367" border="1" cellpadding="1" cellspacing="1" bordercolor="#FFFFFF" bgcolor="EBF1FD">
        <s:form action="articleInfo_article_update">
        <tr>
          <td width="80" height="30">文章标题：</td>
          <td width="470" bgcolor="#FFFFFF"><s:textfield name="title" value="%{#request.articleInfo.title}"/></td>
        </tr>
        <tr>
          <td height="30">文章类别：</td>
          <td bgcolor="#FFFFFF"><s:select name="typeName" list="#request.types" value="%{#request.articleInfo.typeName}"/></td>
        </tr>
        <tr>
          <td height="270">文章内容：</td>
          <td bgcolor="#FFFFFF"><s:textarea name="content" cols="70" rows="20" value="%{#request.articleInfo.content}"/></td>
        </tr>
        <tr>
          <td height="30">&nbsp;</td>
          <td><s:submit value=" 修改文章 "/><s:hidden name="id" value="%{#request.articleInfo.id}"/><s:hidden name="author" value="%{#session.account}"/>&nbsp;&nbsp;&nbsp;<a style="cursor:hand;" onclick="javascript:history.go(-1);">返回</a></td>
        </tr>
        </s:form>
      </table> 
      <br>
